<template>
	<el-container>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="list.apiObj" row-key="id" stripe @selectionChange="selectionChange">
...
export default {
	...
	methods: {
		...
		async batch_del() {
			this.$confirm(
				...
				.then(async () => {
					...
					const res = await this.$API.role.userDelete.delete(ids);
					if (res.code === 0) {
						loading.close();
						this.$message.success("删除成功");
					}
		...
		async onSubmit(form) {
			const res = await this.$API.role.userSave.post(form);
			if (res.code === 0) {
